...
const createWindow = () => {
    ...
    //在词法分析菜单下增加一个词法分析器的子菜单
    menu.items[2].submenu.append(new MenuItem({ //menu.items获取是的主菜单一级菜单的菜单数组，menu.items[2]在这里就是第3个File菜单对象，在其子菜单submenu中添加新的子菜单
        label: "词法分析器",
        click() {
            mainWindow.webContents.send('action', 'compile'); //点击后向主页渲染进程发送“编译”的命令
        },
        role: 'compile',
        accelerator: 'CmdOrShift+F10' //快捷键：Shift+F10
    }));

    //在词法分析菜单下增加一个NFA_DFA_MFA的子菜单
    menu.items[2].submenu.append(new MenuItem({ //menu.items获取是的主菜单一级菜单的菜单数组，menu.items[2]在这里就是第3个File菜单对象，在其子菜单submenu中添加新的子菜单
        label: "NFA_DFA_MFA",
        click() {
            mainWindow.webContents.send('action', 'regular'); //点击后向主页渲染进程发送“正规化”的命令
        },
        role: 'regular',
        accelerator: 'CmdOrShift+F11' //快捷键：Shift+F11
    }));
